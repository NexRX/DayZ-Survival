// Permanent, re-runnable sanity check for the whole trader economy -
// answers "did we miss anything, and is anything mispriced?" without a
// manual eyeball pass. Complements marketGapFill.ts (which *fixes* known
// gaps by classname): this module only reads and reports, it never writes
// to any market file, so it's always safe to run.
//
// Three checks, cross-referencing the mission's merged economy
// (ECONOMY_TYPES_FILE - vanilla + every mod's own types) against what's
// actually sellable (every classname in every category file a custom
// trader identity references - see traders.ts's CUSTOM_TRADER_IDENTITIES):
//
//   Bucket A (high confidence): has a real <category name="..."/> tag in
//     types.xml, not sellable, not excluded/never-sellable - near-certain a
//     real gap that should be added to src/data/marketGapFill.json.
...
//     and decor props alongside the occasional genuine gap, too noisy to
//     auto-flag as a gap, so this bucket is filtered down to "never
//     sellable" patterns first and whatever's left is for a human to
...
} from "../config/market.ts";
import { isExcluded } from "../config/marketGapFill.ts";
import { CUSTOM_TRADER_IDENTITIES } from "../config/traders.ts";

const REPORT_FILE = `${PROFILE_DIR}/market-audit-report.txt`;

// Same regex-scan convention as economy.ts's own TYPE_BLOCK - kept
// consistent even though this read-only tool has no round-tripping
// concerns of its own.
...
// items, just ones this project decided shouldn't be sold).
...
export async function auditMarket(): Promise<void> {
  ...
  // Every category name a custom trader identity actually references - the
  // true "can a player buy/sell this anywhere" list. Used for Bucket C
  ...
  // "Boats" - built/tuned but not wired to a trader yet). Once something
  // has a real, generated category entry it's been reviewed and accounted
  // for - it shouldn't keep cluttering Bucket A/B as if it were an unknown
  // gap just because nobody's attached its category to a trader yet.
  // Bucket C stays scoped to `sellable` only (see above).
