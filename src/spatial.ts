...
// all - they spawn based on proximity to any player, governed by the
// file's global MinDistance/MaxDistance/timer settings.
//
...
// defaults or an admin's tuning. One exception: `Audio_Enabled` defaults to
// 0 out of the box, which makes the addon's own placeholder `Audio` zones
// dead code - so the first time we add a real, positioned `Audio` zone, we
// also flip that switch so it's actually testable.
...
export async function ensureSpatialAI(): Promise<void> {
  ...
  // default) - once we add a real, positioned zone, flip the switch so it
  // actually fires. If an admin later disables it again, we won't fight
  // that choice on subsequent starts.
